...
/**
 * Mobile-only full-height model switcher panel that sits between the header
 * and the chat input with small margins on all sides.
 */
export default function ModelSwitcherPanelMobile(
  props: ModelSwitcherPanelProps
): ReactElement | null {
  const {
    state,
    handlers,
  ...
  const { open, searchQuery, filtersOpen } = state;
  if (!open) {
    return null;
  }
  const {
  ...
  return (
    <div className="model-switcher-panel fixed inset-x-0 top-28 bottom-28 z-40 flex items-center justify-center px-4">
      <div
        className="relative mx-auto flex w-full max-w-sm max-h-[75vh] flex-col rounded-3xl border border-surface bg-surface text-xs ring-1 ring-slate-950/70"
        role="listbox"
      ...
      >
          ...
          <label className="mb-2 flex items-center gap-2 rounded-xl bg-surface px-2 py-1.5 text-[11px] text-muted-foreground">
            ...
            <input
              ...
              onChange={(event: ChangeEvent<HTMLInputElement>): void =>
                handleChangeSearchInternal(event, onChangeSearch)
              }
            ...
            <div className="mt-2">
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {featureOptions.map(
                    (feature: {
                      readonly id: AiModelFilterTag | "all";
                      readonly label: string;
                    }): ReactElement => {
                      const isActiveAll: boolean =
                        feature.id === "all" && featureFilter === "all";
                      ...
                      return (
                        ...
                        >
                          <span
                            aria-hidden="true"
                            className="mr-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-surface-muted"
                          >
        ...
        <div className="flex-1 space-y-0.5 overflow-y-auto py-1 scroll-area chat-scroll-touch">
          ...
          {visibleOptions.length > 0 &&
            visibleOptions.map((option: ModelSwitcherOption): ReactElement => {
              ...
              const providerIcon: ReactElement | null = getProviderIcon(
                option.providerId
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    <div
                      className="flex gap-0.5 text-[10px] text-muted-foreground"
                      aria-hidden="true"
                    >
                      ...
                      {option.tags.includes("fast") && (
                        <Zap className="h-3 w-3" />
                      )}
                      {option.tags.includes("quality") &&
                        !option.tags.includes("fast") && (
                          <Sparkles className="h-3 w-3" />
                        )}
                      {option.tags.includes("local") && (
